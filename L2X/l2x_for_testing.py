...
PARENT_DIR='/home/tyellins/projects/causal_flow/event_causality/hide_and_seek/L2X/synthetic' #Tal added
...
class Sample_Concrete(Layer):
	...
	def get_continuous_samples(self, logits):
		# Tal added: method to get continuous mask
		...
		gumbel = -K.log(-K.log(uniform))
		noisy_logits = (gumbel + logits_)/self.tau0
		return K.max(K.softmax(noisy_logits), axis=1)
...
def L2X(datatype, 
	...
	if train: 
		...
		model.fit(x_train, y_train, validation_data=None,callbacks = callbacks_list, epochs=1, batch_size=BATCH_SIZE) #note - removed val data from original code to confirm no overfitting. ran under both and minor differences. note - only one epoch.
		st2 = time.time() 
	else:
	...
	# for the continuous masks - Tal added - for clustering on the continuous masks. In the end, wasn't better. Not used.
	sample_layer = model.get_layer('sample')
	continuous_samples = Lambda(lambda x: sample_layer.get_continuous_samples(x))(logits)
	continuous_model = Model(model_input, continuous_samples)
...
# 	# sys.argv = ["explain.py", "--train", "--datatype", "orange_skin"]
...
# 	scores = L2X(datatype = args.datatype, 
# 		train = args.train,
